      ...
      document.getElementById('exportLink').href = `export_applicants.php?year=${year}`;
        $.ajax({
                      ...
                      success: function(response) {
                        ...
                        let rows = '';
                        let i = 1; // ตัวแปรลำดับเริ่มจาก 1
                        response.forEach(row => {
                        let paymentStatus = (row.FINANCESTATUS === 'N') ? 'ชำระเงินค่าสมัครแล้ว' : 'ยังไม่ชำระเงินค่าสมัคร';
                        rows += `<tr>
                                    <td style="width: 1rem;">${i++}</td> <!-- เพิ่มลำดับที่ -->
                                    <td style="width: 26rem;">${row.PREFIXNAME}${row.APPLICANTNAME}  ${row.APPLICANTSURNAME}</td>
                                    ...
                                    <td style="width: 20rem;">${row.BYTEDES}</td>
